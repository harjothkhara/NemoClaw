...
An `unhealthy` line that reports HTTP `404` means the endpoint answered but serves no `GET /v1/models` route, so NemoClaw cannot confirm the inference route.
Check that the recorded endpoint is the OpenAI-compatible base URL and not a path below it.
Onboarding reports the same case and exits nonzero.
