...
| `reachable` | The route returned an HTTP status from `200` through `499`, other than `404`, and NemoClaw did not send an inference request. |
| `unhealthy` | The route returned HTTP `404`, HTTP `500` through `599`, another failing status, or an invalid 2xx response body. |
...
An HTTP `404` means the endpoint serves no `GET /v1/models` route, so it does not confirm reachability.
Deep Agents Code on `openrouter-api` is the one exception, because that adapter never serves the model-list route.
...
HTTP responses from `200` through `499`, including `401` and `403`, pass this check, except HTTP `404`.
An HTTP `404` means the endpoint serves no `GET /v1/models` route, so it cannot show that the inference route works.
Deep Agents Code on `openrouter-api` is the one exception, because that adapter never serves the model-list route.
HTTP `404`, HTTP `500` through `599`, interim `100` through `199`, transport failures with status `000`, invalid status values, and an unavailable authoritative probe fail the check.
